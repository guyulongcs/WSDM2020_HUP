...
from PRNN_BLSTM import *
from MicroRNN import *

from process_data.FileTool import *
from process_data.Config import *
from process_data.Metrics import *
from process_data.ProTool import *
from process_data.MLTool import *
from process_data.Data import *


DEBUG_MODE=False
DEBUG_DATA_LINES=128


class PRNN_BLSTM_Rec():
    model_tag_list = ["PRNN_BLSTM", "MicroRNN"]
    def __init__(self):
        #data
        ...
        # config
        self.model_tag = PRNN_BLSTM_Rec.model_tag_list[0]
    ...
    def set_files_config(self, model_tag, layer_num, rnn_state_size, bottom_emb_item_len, flag_embedding_trainable, folder_input, folder_output, exp_sig, init_emb_wgts_bootom_items_path, train_file, test_file, train_len, test_len,
                        ...
                        flag_train, train_epoch, flag_test_candidate_all, mode_attention, drop_out_r, loss_weights, rnn_type, RNN_norm):
        #files
    ...
    def create_model(self):
        ...
        if(self.model_tag == "PRNN_BLSTM"):
            self.hie_rnn_model = PRNN_BLSTM.create_model_hierarchy(self.micro_item_list, self.emb_wgts_micro_items_dict,
                ...
                seq_len=self.seq_len, batch_size=self.batch_size, mode_attention=self.mode_attention, drop_out_r=self.drop_out_r, rnn_type=self.rnn_type, RNN_norm=self.RNN_norm)
    ...
    def train_model(self):
        ...
        #[data_micro, total_data_line_cnt] = Data.load_data_trans_bottomId_to_microItemsId_pickle(self.train_file, self.train_len, self.map_file_bottom_id_to_itemsId, self.micro_item_list)
        ...
        for e in range(n_epoch):
            ...
            for batch_id in range(batch_cnt):
                ...
                dict_emb_sku = self.get_model_wegits_by_layer_name("Embedding0")
                dict_emb_cid3 = self.get_model_wegits_by_layer_name("Embedding2")

                y_true_sku_emb = Data.get_embedding(y_true_sku, dict_emb_sku)
                y_true_cid3_emb = Data.get_embedding(y_true_cid3, dict_emb_cid3)
                ...
                # print model.train_on_batch(x, y_emb)
                sys.stdout.flush()
    ...
    def eval_test_result_example(self, lineNo, skuIdList, xcid3, att_sku, att_cid3, rnn_out_micro_last, dict_top_id_recallIdList, dict_emb, y_pred_emb, y_true_id, y_pred_cid3_emb, y_true_cid3_id, NList, flag_test_candidate_all, dict_emb_sku, dict_emb_cid3):
        #cal similarity
        ...
        if(flag_test_candidate_all):
            ...
            #candidate_id_list = self.get_all_sku_id()
        ...
        for candidate_id in candidate_id_list:
            ...
            candidate_emb = dict_emb_sku[int(candidate_id)]
            sim = np.dot(y_pred_emb, candidate_emb) / (np.linalg.norm(y_pred_emb) * np.linalg.norm(candidate_emb))
            #print "candidate_id:%s, sim:%s" % (str(candidate_id), str(sim))
        ...
        for candidate_id_cid3 in candidate_id_list_cid3:
            ...
            candidate_emb_cid3 = dict_emb_cid3[int(candidate_id_cid3)]
    ...
    def get_model_wegits_by_layer_name(self, layer_name):
        wei =self.hie_rnn_model.get_layer(layer_name).get_weights()[0]
        wei = np.array(wei)
        return wei
    ...
    def test_model(self, NList=[1, 2, 5, 8, 10, 20, 50, 100]):
        ...
        for batch_id in range(batch_cnt):
            ...
            for q, out in enumerate(y_pred_sku):
                ...
                [res_sku, res_cid3] = self.eval_test_result_example(lineNo, xsku, xcid3, att_sku, att_cid3, rnn_out_micro_last, dict_top_id_recallIdList, self.emb_wgts_micro_items_dict, y_pred_sku_emb, y_true_sku_id, y_pred_cid3_emb, y_true_cid3_id, NList, self.flag_test_candidate_all, dict_emb_sku, dict_emb_cid3)
                lineNo += 1
...
if __name__ == '__main__':
    ...
    rnn_type = sys.argv[26]
    RNN_norm = sys.argv[27]


    # model
    hieRNNRec = PRNN_BLSTM_Rec()
    ...
    #if(hieRNNRec.flag_load_model_weights):
    #    hieRNNRec.load_model_weights()
